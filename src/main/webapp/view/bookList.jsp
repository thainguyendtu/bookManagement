	...
	<div align="center">
		...
		<c:choose>
			<c:when test="${not empty userInfo}">
				<a href="${pageContext.request.contextPath}/book/getAllFavoriteBooks">My Book List</a>
				<br>
		...
		<table>
			...
			<tbody>
				<c:forEach var="book" items="${bookList}" varStatus="loop" >
					<tr class="row" style="${(loop.index + 1) % 2 == 0 ? '': 'background: #E3E3ED'}">
						<td style="text-align: center">${loop.index + 1}</td>
						<td class="title"><a href="${pageContext.request.contextPath}/book/detail/${book.id}">${book.title}</a></td>
						<td>${book.author}</td>
						<td>${book.description}</td>
						<td style="text-align: center"><fmt:parseDate var="dateCreatedParsed" value="${book.createdAt}" pattern="yyyy-MM-dd"/>
						<fmt:formatDate value="${dateCreatedParsed}" pattern="dd/MM/yyyy"/></td>
						<td style="text-align: center"><fmt:parseDate var="dateUpdatedParsed" value="${book.updatedAt}" pattern="yyyy-MM-dd"/>
						<fmt:formatDate value="${dateUpdatedParsed}" pattern="dd/MM/yyyy"/></td>
